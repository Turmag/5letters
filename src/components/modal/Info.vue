    ...
    <vue-final-modal v-model="store.isShowInfo" :classes="$style.modal" :content-class="$style.content">
        ...
        <div :class="$style.inner">
            <template v-if="isRules">
                ...
                <div :class="[$style.text, $style.mt]">
                    По поводу буквы <b>ё</b>. Её в этой игре вводить нельзя. Хотя я являюсь сторонником этой буквы, несмотря ни на что! Просто конкретно вот здесь вводить её разрешение не даётся. Однако, если вы всё же захотите и дадите обратную связь по этому поводу, я сделаю разрешение на её ввод ;)
                </div>
                <div :class="[$style.text, $style.mt]">
